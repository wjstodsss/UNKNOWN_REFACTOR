...
<body>
	...
	<div class="content_area">
					...
					<c:forEach items="${cartInfo}" var="ci">
						<tr>
							<td class="td_width_1 cart_info_td"><input type="checkbox"
								...
								<input type="hidden" class="individual_itemPrice_input"
								value="${ci.itemPrice}"> <input type="hidden"
								class="individual_salePrice_input" value="${ci.salePrice}">
								<input type="hidden" class="individual_itemCount_input"
								value="${ci.itemCount}"> <input type="hidden"
								class="individual_totalPrice_input"
								value="${ci.salePrice * ci.itemCount}"> <input
								type="hidden" class="individual_point_input" value="${ci.point}">
								<input type="hidden" class="individual_totalPoint_input"
								value="${ci.totalPoint}"> <input type="hidden"
								class="individual_itemId_input" value="${ci.itemId}"></td>

							<td class="td_width_2">
								<div class="image_wrap" data-itemid="${ci.imageList[0].itemId}"
							...
							<td class="td_width_4 price_td"><del>
									...
									정가 :
									<fmt:formatNumber value="${ci.itemPrice}" pattern=" #,### 원" />
								</del><br> 판매가 : <span class="red_color"><fmt:formatNumber
										value="${ci.salePrice}" pattern=" #,### 원" /></span><br> 포인트: <span
								class="green_color"><fmt:formatNumber value="${ci.point}"
										pattern=" #,### P" /></span></td>
							<td class="td_width_4 table_text_align_center">
								<div class="table_text_align_center quantity_div">
									<input type="text" value="${ci.itemCount}"
							...
							<td class="td_width_4 table_text_align_center"><fmt:formatNumber
									value="${ci.salePrice * ci.itemCount}" pattern="#,### 원" /></td>
							<td class="td_width_4 table_text_align_center">
								<button class="delete_btn" data-cartid="${ci.cartId}">선택삭제</button>
							</td>
						</tr>
					</c:forEach>
				</tbody>
					...
					<tr>
						...
						<td>
							<table>
								<tbody>
									<tr>
										<td><strong>총 적립 예상 포인트</strong></td>
										<td><span class="totalPoint_span">70000</span> P</td>
									</tr>
								</tbody>
							</table>
						</td>
		...
		<div class="content_btn_section">
			<a href="/main" class="continue_btn">쇼핑 계속하기</a> <a class="order_btn">주문하기</a>
		</div>
		...
		<form action="/cart/update" method="post" class="quantity_update_form">
			<input type="hidden" name="cartId" class="update_cartId"> <input
				type="hidden" name="itemCount" class="update_itemCount"> <input
